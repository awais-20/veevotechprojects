 ...
 *   app_id: Application ID (optional, defaults to munshi app ID)
 * }
 * 
 * Standard Error Types:
 * - DB_ERROR: All database-related errors
 * - CACHE_ERROR: All cache-related errors
 * - MQ_ERROR: All message queue-related errors
...
//const rabbitmq_ops = require('../../_core_apps_connectivities/rabbitmq');
//const {rabbitmq_ops} = require('../Core_App_Connectivities/queue');
const event_router = require('../Services/event_router.service');
const rabbitmq_ops = require('../Core_App_Connectivities/Rabitmq')
 ...
 * 
 *********************/
const munshi_app_id = 1022; // OneID Application ID for Munshi Service
...
function register_munshi_event_handler() {
    try {
        ...
        event_router.register_handler('MUNSHI_EVENT', async (event_data) => {
            try {
                console.log(`FILE: munshi.service.js | MUNSHI_EVENT handler | Received error data: ${event_data.event_data.error_title}`);
...
function register_general_log_event_handler() {
    try {
        ...
        event_router.register_handler('GENERAL_LOG_EVENT', async (event_data) => {
            try {
                console.log('uuuuuuuu', event_data);
                console.log(`FILE: munshi.service.js | GENERAL_LOG_EVENT handler | Received log data:${event_data.event_data.log_type} - ${event_data.event_data.plain_text}`);
 ...
 * @param {number} event_data.ttl - Time to live in seconds
 * @param {number} event_data.app_id - Application ID
 * 
 **********/
async function report_to_munshi(event_data) {
    try {
        //console.log(`Event data ha ye: ${event_data}`);
        ...
        const system_log_data = {
            app_id: app_id,
            org_id: event_data.org_id || 1022,
            oneid: event_data.oneid || 1022,
        ...
        try {
            ...
            console.log(`FILE: munshi.service.js | report_to_munshi | Error reported to Munshi queue: ${event_data.event_data.error_code} - ${event_data.event_data.error_title}`);
        } catch (queue_error) {
            console.error(`FILE: munshi.service.js | report_to_munshi | Failed to report to queue, attempting retry:`, queue_error);
            ...
            setTimeout(async () => {
                try {
                    ...
                    await rabbitmq_ops.sendToRabbitMQ(munshi_queue_name, queue_message);
                    console.log(`FILE: munshi.service.js | report_to_munshi | Retry successful for error: ${event_data.error_code}`);
                } catch (retry_error) {
                    console.error(`FILE: munshi.service.js | report_to_munshi | Retry failed for error: ${event_data.error_code}`, retry_error);
                }
            }, 2000); // 2 second delay
        }
    } catch (error) {
 ...
 * @param {number} event_data.app_id - Application ID (optional)
 * 
 **********/
async function publish_general_log(event_data) {
    try {
        console.log(`FILE: munshi.service.js | publish_general_log | Processing general log: ${event_data.event_data.log_type} - ${event_data.event_data.plain_text}`);
       // console.log('rrrrrrr',munshi_app_id);
        // Set default values for unspecified fields
        const app_id = event_data.app_id || munshi_app_id;
        const ttl = event_data.ttl || munshi_ttl;
        
        // Prepare general log data for the queue-based logging system
        const general_log_data = {
            app_id: app_id,
            oneid: event_data.event_data.oneid || null,
            org_id: event_data.event_data.org_id || null,
            entity_id: event_data.event_data.entity_id || null,
            entity_type: event_data.entity_type || null,
            log_type: event_data.event_data.log_type,
            plain_text: event_data.event_data.plain_text,
            metadata: {
       ...
       console.log('iiiiiiii:', general_log_data);
        // Publish to the queue-based logging system
        try {
            // Add log type and timestamp for queue processing
            const queue_message = {
                ...general_log_data,
                log_type: event_data.event_data.log_type, // Preserve the original log_type for general logs
                queued_at: Math.floor(Date.now() / 1000)
            };
            //console.log('jjjjj',queue_message)
            await rabbitmq_ops.sendToRabbitMQ(munshi_queue_name, queue_message);
         
            console.log(`FILE: munshi.service.js | publish_general_log | General log published to Munshi queue: ${event_data.event_data.log_type} - ${event_data.event_data.plain_text}`);
        } catch (queue_error) {
            console.error(`FILE: munshi.service.js | publish_general_log | Failed to publish to queue, attempting retry:`, queue_error);
            ...
            setTimeout(async () => {
                try {
                    ...
                    await rabbitmq_ops.sendToRabbitMQ(munshi_queue_name, queue_message);
                    console.log(`FILE: munshi.service.js | publish_general_log | Retry successful for log: ${event_data.event_data.log_type}`);
                } catch (retry_error) {
                    console.error(`FILE: munshi.service.js | publish_general_log | Retry failed for log: ${event_data.event_data.log_type}`, retry_error);
                }
            }, 2000); // 2 second delay
        }
    } catch (error) {
...
function extract_stack_trace(error) {
    try {
        console.log(`FILE: munshi.service.js | extract_stack_trace | Extracting stack trace information`,error);
 ...
 * @param {number} app_id - Application ID (optional)
 */
function report_error_to_munshi(error, error_code, error_title, error_type, metadata = {}, other_data = {}, nonblocking = 0, ttl = null, app_id = null) {
    try {
 ...
 * @param {number} app_id - Application ID (optional)
 */
function log_general_activity(log_type, plain_text, oneid = null, org_id = null, entity_id = null, entity_type = null, metadata = {}, changes = [], other_data = {}, ttl = null, app_id = null) {
    try {
